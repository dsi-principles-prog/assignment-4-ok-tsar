...
sum_by_state <- function(df, vars) {
  df %>% 
    group_by(state) %>% 
    mutate_at(vars, list(state_sum = function(x, na.rm = FALSE) {sum(x, na.rm = TRUE)})) %>% 
    ungroup() %>% 
...
```{r per_1M_by_state}
per_1M_by_state <- function(df) {
  df %>%
    group_by(state) %>%
    mutate(
      killed_per_1M = (killed_state_sum / population) * 1000000,
      wounded_per_1M = (wounded_state_sum / population) * 1000000
    ) %>%
    ungroup() %>%
    select(state, contains("per_1M"), everything())
}
```
